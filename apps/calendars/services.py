...
def is_next_year(date1, date2):
    ...
    next_year = date1 + relativedelta(years=1)
...
def repeat_it_by_day(self):
    ...
    elif self.repeat_end_date and is_next_day(self.start_date, self.repeat_end_date):
        
        curr_date = self.start_date
        while is_next_day(curr_date, self.repeat_end_date):
            new_event = self.__class__(
                name=self.name,
...
def repeat_it_by_week(self):
    ...
    if self.occurrence and self.occurrence > 1:
        ...
        curr_date = self.start_date
        while is_next_week(curr_date, self.repeat_end_date):
            new_event = self.__class__(
                name=self.name,
                start_date=curr_date,
                end_date=self.actual_end_date + (curr_date - self.start_date),
...
def repeat_it_by_month(self):
    ...
    elif self.repeat_end_date and is_next_month(self.start_date, self.repeat_end_date):
...
def repeat_it_by_year(self):
    ...
    elif self.repeat_end_date and is_next_year(self.start_date, self.repeat_end_date):

        curr_date = self.start_date
        while is_next_year(curr_date, self.repeat_end_date):
